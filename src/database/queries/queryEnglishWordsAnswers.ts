...
function prepareSqlStatement(wordsIdsCount: number) {
  return `
    SELECT LOOKUP.polish_word AS id, LOOKUP.english_word AS questionId, PL.word AS value
    FROM words_lookup AS LOOKUP
    INNER JOIN polish_words AS PL ON PL.id=LOOKUP.polish_word
    WHERE LOOKUP.english_word IN (${new Array(wordsIdsCount).fill('?').join(', ')})
  `;
}
...
export default async function queryEnglishWordsAnswers(db: WebSQLDatabase, wordsIds: number[]): Promise<IAnswer[]> {
  ...
  for (let i = 0; i < results.rows.length; i++) {
    ...
    try {
      ...
      throw new CorruptedDatabaseException(
        "Couldn't fetch `answers for english-words` query results: " + getValidationErrorMessage(e)
      );
    }
  }
